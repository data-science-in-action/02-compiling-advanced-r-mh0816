...
#> 1 mean1(x)      313μs    356μs     2638.
#> 2 mean2(x)      153μs    163μs     5668.
```
...
#>   expression           min   median `itr/sec`
#>   <bch:expr>      <bch:tm> <bch:tm>     <dbl>
#> 1 mean(x)            4.8μs    7.3μs   117790.
#> 2 mean.default(x)    1.9μs    2.7μs   316538.
```
...
#>   expression              min   median `itr/sec`
#>   <bch:expr>         <bch:tm> <bch:tm>     <dbl>
#> 1 mean(x)               4.8μs    6.5μs   141210.
#> 2 mean.default(x)       1.8μs    2.4μs   357893.
#> 3 .Internal(mean(x))    400ns    600ns  1474144.
```
...
#>   expression              min   median `itr/sec`
#>   <bch:expr>         <bch:tm> <bch:tm>     <dbl>
#> 1 mean(x)              32.7μs   38.6μs    23846.
#> 2 mean.default(x)      31.4μs     34μs    27193.
#> 3 .Internal(mean(x))   28.2μs   30.9μs    28399.
```
...
#>   expression         min   median `itr/sec`
#>   <bch:expr>    <bch:tm> <bch:tm>     <dbl>
#> 1 as.data.frame   1.38ms   1.67ms      505.
#> 2 quick_df         7.8μs   10.2μs    85385.
```
...
#>   expression        min   median `itr/sec`
#>   <bch:expr>   <bch:tm> <bch:tm>     <dbl>
#> 1 lookup[x1]      600ns      1μs   895151.
#> 2 lookup[x10]     1.4μs    2.4μs   358892.
#> 3 lookup[x100]    2.7μs    6.7μs   129442.
```
...
#>   expression      min   median `itr/sec`
#>   <bch:expr> <bch:tm> <bch:tm>     <dbl>
#> 1 loop10       21.1μs   34.5μs    25447.
#> 2 loop100     673.7μs  848.6μs     1045.
#> 3 vec10         4.8μs      7μs   127349.
#> 4 vec100       33.5μs   41.5μs    20776.
```
